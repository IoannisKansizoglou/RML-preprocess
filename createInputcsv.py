...
def read_data(datasetPATH):
    ...
    centerframesPATH, spectrogramsPATH, labels = list(), list(), list()
                ...
                if language != 'Thumbs.db':
                    ...
                    for vid in videos:
                        ...
                        for spectrogram in spectrograms:
                            ...
                            # Append path of spectrogram
                            spectrogramsPATH.append(spectrogramPATH)
                            ...
                            if os.path.isfile(centerframePATH):
                                ...
                                # Append path of frame
                                centerframesPATH.append(centerframePATH)
                            else:
    ...
    data = np.array([centerframesPATH, spectrogramsPATH, labels])
    data = data.T
...
### Function to unroll faces from center frames, spectrograms and labels to numpy arrays ###
def unroll_data(data):

    # Split columns of data matrix
    centerframesPATH = data.T[0]
    valued_spectrogramsPATH = data.T[1]
    valued_labels = data.T[2]
    # List to append all faces paths
    facesPATH = list()
    # Run through all center frames
    for centerframePATH in centerframesPATH:
    ...
    # Convert faces path to numpy array
    facesPATH = np.array(facesPATH)
    
    spectrogramsPATH = [None] * np.shape(valued_spectrogramsPATH)[0]*num_frames
    spectrogramsPATH[:np.shape(valued_spectrogramsPATH)[0]] = valued_spectrogramsPATH
    labels = [None] * np.shape(valued_labels)[0]*num_frames
    labels[:np.shape(valued_labels)[0]] = valued_labels
    return facesPATH, spectrogramsPATH, labels
...
def main():
    ...
    train_faces, train_spectrograms, train_labels = unroll_data(train_data)
    eval_faces, eval_spectrograms, eval_labels = unroll_data(eval_data)
    #pred_faces, pred_spectrograms, pred_labels = unroll_data(pred_data)
    ...
    create_folder(datasetPATH+'Core')

    df1 = pd.DataFrame(data=d1)
    df2 = pd.DataFrame(data=d2)
    #df3 = pd.DataFrame(data=d3)
    df1.to_csv( datasetPATH + 'Core/classification/training_data.csv' )
    df2.to_csv( datasetPATH + 'Core/classification/evaluation_data.csv' )
    #df3.to_csv( datasetPATH + 'Core/prediction_data.csv' )
